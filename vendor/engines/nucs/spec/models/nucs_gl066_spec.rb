...
RSpec.describe NucsGl066 do
  ...
  it "should tell us when now is before a starts_at date" do
    gl = FactoryGirl.create(:nucs_gl066_with_dates, starts_at: Time.zone.now + 5.days, expires_at: Time.zone.now + 8.days)
    expect(gl).to be_expired
  end

  it "should tell us when now is after a expires_at date" do
    gl = FactoryGirl.create(:nucs_gl066_with_dates, starts_at: Time.zone.now - 5.days, expires_at: Time.zone.now - 2.days)
    expect(gl).to be_expired
  end

  it "should tell us when now is in a starts_at and expires_at window" do
    gl = FactoryGirl.create(:nucs_gl066_with_dates, starts_at: Time.zone.now - 3.days, expires_at: Time.zone.now + 3.days)
    expect(gl).not_to be_expired
  end

  it "should tell us when now on starts_at" do
    gl = FactoryGirl.create(:nucs_gl066_with_dates, starts_at: Time.zone.now, expires_at: Time.zone.now + 3.days)
    expect(gl).not_to be_expired
  end

  it "should tell us when now on expires_at" do
    gl = FactoryGirl.create(:nucs_gl066_with_dates, starts_at: Time.zone.now - 3.days, expires_at: Time.zone.now.end_of_day)
    expect(gl).not_to be_expired
  end
