...
RSpec.describe "Managing Certificates" do
  ...
  let(:staff_member) { FactoryGirl.create(:user, :staff, facility: facility) }
  ...
  let!(:other_certificate) { FactoryGirl.create(:certificate) }
  let!(:extra_certificate) { FactoryGirl.create(:certificate) }
