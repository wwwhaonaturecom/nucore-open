namespace :nu do
  ...
  task find_cancer_center_order_details: :environment do |_t, _args|
    import DateHelper
    ...
    order_details.each_with_index do |od, _i|
      ...
      next unless od.price_policy.price_group_id == cancer_center_group.id && od.price_policy != original_price_policy
      if [23_269, 26_291, 26_172, 26_480].include? od.id
  ...
  task update_order_details_46319: :environment do |_t, _args|
    od_to_price = {
      7624 => 90.0,
      7901 => 25.50,
      8939 => 7.40,
  ...
  task add_activity_01: :environment do |_t, _args|
  ...
  task correct_auto_cancelled_orders: :environment do |_t, _args|
  ...
  task provision_users: :environment do |_t, _args|
  ...
  task remove_mhpl_orders: :environment do |_t, _args|
    order_ids = []
  ...
  task update_order_details_76683: :environment do
    [120_822, 120_059, 117_873, 116_361, 115_303].each do |id|
    ...
    File.open(File.join(Rails.root, "tmp", "pathcore.csv"), "w+") do |csv|
      ...
      order_details.each do |od|
        if od.price_policy.nil?
          puts "Order detail ##{od} does not have a price policy"
          next
        end
  ...
  task fix_accounts_77103: :environment do
    Account.where(id: [9253, 10_453]).each do |account|
